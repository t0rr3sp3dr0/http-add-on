...
type table struct {
	...
	httpScaledObjectInformer                 cache.SharedIndexInformer
...
func NewTable(sharedInformerFactory externalversions.SharedInformerFactory, namespace string) (Table, error) {
	...
	t := table{
		...
		httpScaledObjectInformer: httpScaledObjects.Informer(),
		httpScaledObjects:        make(map[types.NamespacedName]*httpv1alpha1.HTTPScaledObject),
		memorySignaler:           NewSignaler(),
	}

	registration, err := t.httpScaledObjectInformer.AddEventHandler(&t)
	if err != nil {
		return nil, err
	}
...
func (t *table) runInformer(ctx context.Context) error {
	...
	case <-ctx.Done():
		return ctx.Err()
	default:
		return errors.New("The sharedIndexInformer has started, run more than once is not allowed")
	}
}
...
func (t *table) Start(ctx context.Context) error {
	...
	eg.Go(applyContext(ctx, t.runInformer))
	eg.Go(applyContext(ctx, t.refreshMemory))
	return eg.Wait()
}
